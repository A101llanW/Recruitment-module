...
namespace HR.Web.Services
{
    public class CustomReportService : IDisposable
    {
        ...
        private CustomReportPage RunApplications(ValidatedCustomReport config, int page, bool forExport)
        {
            ...
            if (forExport)
            {
                var exportRows = query.Take(ExportRowCap + 1).ToList();
                if (exportRows.Count > ExportRowCap)
                {
                    return new CustomReportPage
                    {
                        ...
                        PageSize = PageSize,
                        HasMore = false,
                        EmptyMessage = "Narrow the filters. Exports are limited to 5000 rows."
                    };
                }
            ...
            var skip = (page - 1) * PageSize;
            var pageItems = query.Skip(skip).Take(PageSize + 1).ToList();
            var hasMore = pageItems.Count > PageSize && page < MaxPreviewPage;
        ...
        private CustomReportPage RunApplicantsPerPosition(ValidatedCustomReport config, int page, bool forExport)
        {
            ...
            }).Take(ExportRowCap + 1).ToList();

            if (forExport && projected.Count > ExportRowCap)
            {
                return new CustomReportPage
                {
                    Headers = headers,
            ...
            if (!forExport && projected.Count > ExportRowCap)
            {
                projected = projected.Take(ExportRowCap).ToList();
                warning = "Counts use the first 5000 matching applications. Narrow the filters for an exact count.";
            }
            ...
            grouped = ApplyAggregateSort(grouped, config).ToList();
