...
namespace HR.Web.Services
{
    public static class CustomReportConfigValidator
    {
            ...
            out string error)
        {
            ...
            var rowLimit = parsed.Filters != null && parsed.Filters.RowLimit.HasValue
                ? parsed.Filters.RowLimit.Value
                : DefaultRowLimit;
            if (rowLimit < MinRowLimit || rowLimit > MaxRowLimit)
            {
                error = string.Format("Row limit must be between {0} and {1}.", MinRowLimit, MaxRowLimit);
                return false;
            }
